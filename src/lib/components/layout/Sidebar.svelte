...
{#if !$mobile && !$showSidebar}
	<div
		class="gpthub-sidebar-rail pt-[7px] pb-2 px-2 flex flex-col justify-between text-black dark:text-white hover:bg-gray-50/30 dark:hover:bg-gray-950/30 h-full z-10 transition-all border-e-[0.5px] border-gray-50 dark:border-gray-850/30"
		id="sidebar"
	>
		<button
...
{#if $showSidebar}
	<div
		...
		id="sidebar"
		class="gpthub-sidebar-panel h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
	...
	>
		<div
			class="gpthub-sidebar-inner my-auto flex flex-col justify-between h-screen max-h-[100dvh] w-[var(--sidebar-width)] overflow-x-hidden scrollbar-hidden z-50 {$showSidebar
				? ''
				: 'invisible'}"
		>
			<div
				class="gpthub-sidebar-header sidebar px-[0.5625rem] pt-2 pb-1.5 flex justify-between space-x-1 text-gray-600 dark:text-gray-400 sticky top-0 z-10 -mb-3"
			>
				<a
