...
<div
	class="gpthub-chat-shell h-screen max-h-[100dvh] transition-width duration-200 ease-in-out {$showSidebar
		...
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			...
			<PaneGroup direction="horizontal" class="w-full h-full">
				<Pane defaultSize={50} minSize={30} class="h-full flex relative max-w-full flex-col">
					...
					<div
						id="chat-pane"
						class="gpthub-chat-pane flex flex-col flex-auto z-10 w-full @container overflow-auto"
					>
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
							<div
								class="gpthub-messages-container pb-2.5 flex flex-col justify-between w-full flex-auto overflow-auto h-0 max-w-full z-10 scrollbar-hidden"
